	...
	public class MeetingStartTime
	{
		...
		public static MeetingStartTime FromText(string text)
		{
			...
			if (text.Contains(";"))
			{
				...
				meetingDescription = splited[1];

				if (splited.Length > 2)
				{
					congregation = splited[2];
				}
		...
		{
			var result = StartTime.ToString(@"hh\:mm");
			...
			if (!string.IsNullOrWhiteSpace(MeetingDescription))
			{
				result += $";{MeetingDescription}";
			}
